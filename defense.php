...
try
{
 ...
 for($serieNo=1; $serieNo<257; $serieNo++){	
 	...
 	for($i=1; $i<9; $i++){
		$team = $league->getTeam($i);
 		...
 		if($homeTeam->getId()==$teamId){
            ...
            $defense[$homeTeam->getName()]=$homeTeam->getLeftDefenseRating()+
            							   $homeTeam->getCentralDefenseRating()+
                                           $homeTeam->getRightDefenseRating();
            ...
            $defense[$awayTeam->getName()]=$awayTeam->getLeftDefenseRating()+
            							   $awayTeam->getCentralDefenseRating()+
                                           $awayTeam->getRightDefenseRating();
    ...
    echo 'number of teams scanned: ' . count($defense);
    echo "<table style='width:50%'>";
    echo "<tr>";
    echo	"<td></td>";

    echo	"<td> Team       </td>";
    echo    "<td> Defense Value </td>";
    echo "</tr>";   
    $i=0;
    foreach($defense as $x=>$x_value) {
    $i+=1;
    echo "<tr>";
    echo "<td>  $i       </td>";
    echo	"<td> $x       </td>";
