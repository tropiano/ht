...
try
{
 ...
 for($serieNo=1; $serieNo<257; $serieNo++){	
 	...
 	for($i=1; $i<9; $i++){
        ...
        $match = $HT->getSeniorMatchDetails($matchId);
 		...
 		if($homeTeam->getId()==$teamId){
			...
			$attack[$homeTeam->getName()]=$homeTeam->getLeftAttackRating() + 
            							  $homeTeam->getRightAttackRating() + 
                                          $homeTeam->getCentralAttackRating();
            ...
            $attack[$awayTeam->getName()]=$awayTeam->getLeftAttackRating() + 
            							  $awayTeam->getRightAttackRating() + 
                                          $awayTeam->getCentralAttackRating();
    ...
    echo 'number of teams scanned: ' . count($attack);
    echo "<table style='width:50%'>";
    echo "<tr>";
    echo	"<td></td>";

    echo	"<td> Team       </td>";
    echo    "<td> Attack Value </td>";
    echo "</tr>";   
    $i=0;
    foreach($attack as $x=>$x_value) {
    $i+=1;
    echo "<tr>";
    echo "<td>  $i       </td>";
    echo	"<td> $x       </td>";
